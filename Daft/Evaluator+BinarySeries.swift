...
extension Evaluator {
    ...
    class func evaluateBinarySeriesExpression(_ expression: ASTBinarySeriesExpression, stack: Stack) throws -> RValue {
        ...
        return try evaluatePostfix(items: postfix, evaluate: evaluateBinaryOperator)
    }
...
extension Evaluator {
    ...
    fileprivate enum ExpressionItem {
        case value(ValueType)
        case op(BinaryOperator)
    }

    fileprivate enum Associativity {
        case left
        case right
    }

    fileprivate class func precedence(for op: BinaryOperator) -> Int {
        switch op {
    ...
    fileprivate class func associativity(for op: BinaryOperator) -> Associativity {
        switch op {
    ...
    fileprivate class func interleave(values: [ValueType], operators: [BinaryOperator]) throws -> [ExpressionItem] {
    ...
    fileprivate class func convertToPostfix(items: [ExpressionItem], precedence: (BinaryOperator) -> Int, associativity: (BinaryOperator) -> Associativity) -> [ExpressionItem] {
        var input = items
        var output = [ExpressionItem]()
    ...
    fileprivate class func evaluatePostfix(items: [ExpressionItem], evaluate: (BinaryOperator, ValueType, ValueType) throws -> RValue) throws -> RValue {
        var input = items
        var stack = [ValueType]()
